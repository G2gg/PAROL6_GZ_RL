...
Publishes 19D observations matching Isaac Lab format:
- Joint positions (6D)
- Joint velocities (6D)
- End-effector position relative to base (3D) - via TF2
...
class ObservationPublisher(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('use_fk', True)  # Use forward kinematics instead of TF
        ...
        # Observation publisher (19D vector)
        ...
        self.get_logger().info('=== Observation Publisher Started ===')
    ...
    def publish_observations(self):
        """
        Publish complete 19D observation vector.
        ...
        observation = np.concatenate([
            self.joint_pos,        # 6D: joint positions
            self.joint_vel,        # 6D: joint velocities
            ee_pos_rel,            # 3D: end-effector position (relative)
            self.target_pos_rel,   # 3D: target position (relative)
            [distance]             # 1D: distance to target
        ])
        ...
        if self._pub_count % 1000 == 0:  # Every 10 seconds at 100Hz
            self.get_logger().info(
                f'Published {self._pub_count} observations | '
